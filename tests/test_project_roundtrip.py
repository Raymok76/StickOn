...
def test_pur_image_roundtrip() -> None:
...
def test_pur_window_geometry_roundtrip() -> None:
    if QApplication.instance() is None:
        QApplication([])
    scene = QGraphicsScene()
    pm = QPixmap(4, 4)
...
def test_pur_canvas_view_state_roundtrip() -> None:
    if QApplication.instance() is None:
        QApplication([])
    scene = QGraphicsScene()
    pm = QPixmap(4, 4)
...
def test_pur_image_scale_origin_roundtrip() -> None:
    if QApplication.instance() is None:
        QApplication([])
    scene = QGraphicsScene()
    pm = QPixmap(10, 10)
